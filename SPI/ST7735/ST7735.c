...
static volatile LCD_IF_STATUS g_master_event_flag;    // Master Transfer Event completion flag

void ST7735_IF_callback(LCD_IF_STATUS arg)
{
    g_master_event_flag = arg;
}
#endif
...
#endif
    ST7735_IOPin_Low(ST7735_LCD_CS);
}
...
#endif
    ST7735_IOPin_High(ST7735_LCD_CS);
}

void ST7735_Reset()
{
    ST7735_IOPin_Low(ST7735_LCD_RESET);
    ST7735_DelayIF(5);
    ST7735_IOPin_High(ST7735_LCD_RESET);
}
...
#endif
    ST7735_IOPin_Low(ST7735_LCD_AO);
    ST7735_WriteIF(&cmd, sizeof(cmd));
}
...
void ST7735_WriteData(uint8_t* buff, size_t buff_size)
{
    ...
    g_master_event_flag = LCD_SPI_DTAT_TRANSFER;
#endif
    ST7735_IOPin_High(ST7735_LCD_AO);
    ST7735_WriteIF(buff, buff_size);
}
...
void ST7735_FillRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
    ...
    for(y = h; y > 0; y--) {
        for(x = w; x > 0; x--) {
            ...
            g_master_event_flag = LCD_SPI_DTAT_TRANSFER;
#endif
            ST7735_WriteIF(data, sizeof(data));
        }
    }
